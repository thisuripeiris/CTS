...
export default function View_Service_Delete_Issue() {
    ...
    const fetchpayments = () => {
        axios.get(`http://localhost:5001/Service_Delete_ISSUE/`)
            .then((res) => {
    ...
    const handleDelete = (ID, id) => {
        axios.delete(`http://localhost:5001/Service_Payment/delete/${ID}`)
            .then((res) => {
                alert("Successfully deleted Service Payment"); // Fixed: Corrected alert message
                setsalary(prevSalaries => prevSalaries.filter(salary => salary._id !== id));
                axios.delete(`http://localhost:5001/Service_Delete_ISSUE/delete/${id}`)
                    .then((res) => {
                        alert("Successfully deleted Service Payment ISSUE"); // Fixed: Corrected alert message
                        setsalary(prevSalaries => prevSalaries.filter(salary => salary._id !== id));
                        window.location.href = '/View_Service_Delete_Issue'; // Redirect to View_salary_Issue page
