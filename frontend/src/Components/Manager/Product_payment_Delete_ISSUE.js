...
export default function View_Product_Delete_Issue() {
    ...
    const fetchpayments = () => {
        axios.get(`http://localhost:5001/Product_Delete_ISSUE/`)
            .then((res) => {
    ...
    const handleDelete = (ID, id) => {
        axios.delete(`http://localhost:5001/Product_Payment/delete/${ID}`)
            .then((res) => {
                alert("Successfully deleted Product Payment"); // Fixed: Corrected alert message
                setsalary(prevSalaries => prevSalaries.filter(salary => salary._id !== id));
                axios.delete(`http://localhost:5001/Product_Delete_ISSUE/delete/${id}`)
                    .then((res) => {
                        alert("Successfully deleted employee salary Payment ISSUE"); // Fixed: Corrected alert message
                        setsalary(prevSalaries => prevSalaries.filter(salary => salary._id !== id));
                        window.location.href = '/View_Product_Delete_Issue'; // Redirect to View_salary_Issue page
